...
export function* makeMove ({ payload: { row, column } }) {
    try {
        ...
        yield put(gameActions.makeMove(row, column, state.caption.get('playerSign')));

        const response = yield apply(api, api.makeMove, [{row, column}]);
        const { error, boardState, isOver, playerSign, winner  } = yield apply(response, response.json);
